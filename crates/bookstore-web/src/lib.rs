...
async fn admin_cover_upload(
    ...
    require_same_origin(&headers)?;
    let storage = state.cover_storage.clone().ok_or(axum::http::StatusCode::SERVICE_UNAVAILABLE)?;
        ...
        multipart.next_field().await.map_err(|_| axum::http::StatusCode::BAD_REQUEST)?
    {
        match field.name().unwrap_or_default() {
            ...
            "file" => {
                ...
                file_bytes =
                    field.bytes().await.map_err(|_| axum::http::StatusCode::BAD_REQUEST)?.to_vec();
            }
            _ => {}
        }
    }
